#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""NASDAQ 심층분석 PDF — 2026-07-23 (41종목) + Chase 스냅샷 + 기술적분석 훅."""
...
RANK_PATH = Path("/tmp/rank_20260723.json")
QUAL_PATH = Path("/tmp/qual_20260723.json")

HTML_OUT = Path("/workspace/reports/NASDAQ_Deep_Analysis_2026-07-23.html")
PDF_WORKSPACE = Path("/workspace/reports/NASDAQ_Deep_Analysis_2026-07-23.pdf")
PDF_ARTIFACT = Path("/opt/cursor/artifacts/NASDAQ_Deep_Analysis_2026-07-23.pdf")
PDF_ASSETS = Path("/workspace/assets/NASDAQ_Deep_Analysis_2026-07-23.pdf")
CHASE_OUT = Path("/workspace/reports/chase_rr_20260723.json")
...
NEW_Q = {
    "MU": {
        ...
        "sector": "IT·반도체 메모리",
        "feat": "HBM·DRAM AI 수요",
        "item": "메모리 반도체. 한줄: AI 서버용 HBM·DRAM 공급",
        "fin": "AI 메모리 사이클 강세·실적 연속 Beat",
        "strat": "메가캡·업사이드大·추격보다 관심",
        "news": "HBM 공급·AI CapEx 연동",
        "bull": "$1200–1500",
        "base": "$900–1100",
        "bear": "$650–800",
        "brk": "메모리 가격 급락·CapEx 둔화",
        "eps": "4/4 Beat",
        "entry": "눌림 관망",
        "drop": "가이던스 컷",
        "chase": "중상·메가캡",
    ...
    "STX": {
        ...
        "sector": "IT·스토리지",
        "feat": "HDD·데이터센터 스토리지",
        "item": "하드디스크·스토리지. 한줄: 대용량 HDD·클라우드 스토리지",
        "fin": "연속 Beat·고단가",
        "strat": "메가캡 근접·추격 제한",
        "news": "데이터센터 스토리지 수요",
        "bull": "$1000–1100",
    ...
    "LASR": {
        ...
        "sector": "IT·레이저/포토닉스",
        "feat": "고출력 반도체 레이저",
        "item": "산업·방산 레이저. 한줄: 반도체·방산용 고출력 레이저",
        "fin": "EPS 4/4 Beat · PT 업사이드",
        "strat": "서프라이즈·업사이드 정합 → 상",
        "news": "~08-06 실적",
        "bull": "$90–110",
        "base": "$75–90",
        "bear": "$55–70",
    ...
    "VISN": {
        ...
        "sector": "IT·통신장비",
        "feat": "네트워킹·통신장비 · 소형",
        "item": "통신·네트워크 장비. 한줄: 통신장비 소형주 모멘텀",
        "fin": "EPS 4/4 Beat · PT +39%",
        "strat": "업사이드大·유동성·희석 리스크 점검",
        "news": "~08-06 실적",
        "bull": "$16–20",
        "base": "$13–16",
        "bear": "$8–11",
    ...
    "DDOG": {
        "name": "Datadog",
        "sector": "IT·클라우드 관측",
        "feat": "Observability SaaS",
        "item": "클라우드 모니터링. 한줄: 인프라·APM·로그 통합 관측",
        "fin": "EPS 4/4 Beat · PT≈현재",
        "strat": "품질高·밸류 타이트 → 눌림",
        "news": "~08-06 실적",
        "bull": "$280–320",
        "base": "$240–270",
    ...
    "AEHR": {
        ...
        "sector": "IT·반도체 테스트",
        "feat": "웨이퍼 레벨 번인·SiC",
        "item": "반도체 테스트 장비. 한줄: SiC·웨이퍼 번인 테스트",
        "fin": "EPS 2/4 · PT 업사이드",
        "strat": "변동성大·사이즈 제한",
        "news": "~10-08 실적(원거리)",
        "bull": "$120–150",
    ...
    "COCO": {
        ...
        "sector": "소비재·음료",
        "feat": "코코넛워터 브랜드",
        "item": "코코넛워터. 한줄: 비타코코 브랜드 음료",
        "fin": "EPS 4/4 Beat · 07-23 실적 임박",
        "strat": "D-1 이벤트 · 추격 금지·반만",
        "news": "07-23 실적",
        "bull": "$80–90",
        "base": "$70–78",
        "bear": "$55–65",
    ...
    "ICHR": {
        ...
        "sector": "IT·반도체 장비 부품",
        "feat": "가스·유체 딜리버리",
        "item": "반도체 공정 부품. 한줄: 팹 장비 유체·유체 공급 모듈",
        "fin": "EPS 2/4 · PT 소폭 프리미엄",
        "strat": "혼조 · 관망",
        "news": "~08-03 실적",
        "bull": "$100–120",
        "base": "$85–100",
...
# Portfolio-aware Chase label overrides (final display) — 2026-07-23 아침 포트
LABEL_OVERRIDE = {
    "SLS": "상·위성·바이너리",
    "LASR": "상·08-06",
    "NEO": "최상·07-28이벤트·보유",
...
def main() -> None:
    ...
    <section class="cover">
      <h1>NASDAQ 심층분석</h1>
      <p class="sub">Chase Risk-Reward · 실적 서프라이즈 · 41종목</p>
      ...
      <div class="portfolio-box" style="text-align:left;max-width:520px;margin:24px auto;">
        <b>포트 실행 메모 (7/23 아침)</b><br/>
        보유: NEO18 · ECPG2 · AMRX8 · MU0.14 · ASTH3 · NESR3 · INDV1<br/>
        이벤트: NEO 07-28 ≫ AMRX 07-30 · ALKS 07-28 워치<br/>
        익절/손절: portfolio_watch.yaml (STOP/TP/NO_ADD 훅)<br/>
        NESR: $25–27 밴드만 · ECPG/ASTH/INDV/AMRX/MU 추가금지<br/>
        과열·고점군 추격 금지 · 버퍼 현금 유지
      </div>
    </section>
