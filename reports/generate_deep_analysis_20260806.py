#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""NASDAQ 심층분석 PDF — 2026-08-06 (55종목 · 매수시나리오 A′ 점검) + Chase 스냅샷 + 기술적분석 훅."""
...
RANK_PATH = Path("/workspace/reports/rank_20260806.json")
QUAL_PATH = Path("/workspace/reports/qual_20260806.json")

HTML_OUT = Path("/workspace/reports/NASDAQ_Deep_Analysis_2026-08-06.html")
PDF_WORKSPACE = Path("/workspace/reports/NASDAQ_Deep_Analysis_2026-08-06.pdf")
PDF_ARTIFACT = Path("/opt/cursor/artifacts/NASDAQ_Deep_Analysis_2026-08-06.pdf")
PDF_ASSETS = Path("/workspace/assets/NASDAQ_Deep_Analysis_2026-08-06.pdf")
CHASE_OUT = Path("/workspace/reports/chase_rr_20260806.json")
...
def load_buy_scenarios() -> list[dict]:
    p = Path("/workspace/reports/buy_scenarios_20260806.json")
    if not p.exists():
        return []
...
def main() -> None:
    ...
    <section class="cover">
      <h1>NASDAQ 심층분석</h1>
      <p class="sub">Chase Risk-Reward · 매수시나리오 A/B 점검 · 55종목 · A′ 게이트</p>
      ...
      <div class="portfolio-box" style="text-align:left;max-width:560px;margin:16px auto;">
        ...
        오늘(US 8/6) LASR·TXG·ROKU 등 AMC — KST 8/7 새벽 · 갭추격·물타기 금지<br/>
        ECPG/RELY는 8/5 AMC(KST 8/6 새벽) 소화 구간 · 홀드만<br/>
        유니버스 55종(신규 APPS·FLYW·EXEL · CMPR 미포함) · A′에 보유(ECPG/RELY/AMRX) 포함 시 NO_ADD 우선 · 실행은 포폴() 3레이어·등급한도 후
      </div>
    </section>
    {buy_scenario_section(scenarios)}
    <h2>Surprise / Chase 요약</h2>
    <p class="small">정렬: Chase RR. 가격·PT는 {DATE_TAG} 라이브. 55종 · 신규/교체: APPS·FLYW·EXEL · (dropped VISN·MRCY·XMTR·ANDE·ENLT·CRDO·NBIS·IART).<br/>
    ...
    <section class="ticker-section">
      ...
      <p class="small">다음 단계: 기술적분석() ← chase_rr_20260806.json</p>
    </section>
    </body></html>
    """
