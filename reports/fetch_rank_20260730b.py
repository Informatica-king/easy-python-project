...
ASOF = date(2026, 7, 30)
TICKERS = [
    "TXG", "TVTX", "CDNA", "LIND", "FTRE", "AMRX", "VCYT", "KRYS", "JAZZ", "ADPT",
    "FA", "INDV", "ROKU", "CMPR", "GPRE", "EXPE", "RELY", "CRSR", "INCY", "STLD",
    "KNSA", "SBLK", "VTRS", "CASY", "ECPG", "CBRL", "SENEA", "PEBO", "FTNT", "BTSG",
    "DDOG", "HST", "PGNY", "CHEF", "CNXN", "SCSC", "PAGP", "LQDA", "DNTH", "CLMT",
    "PTGX", "ATLC", "SEPN", "RAPP", "ALKS", "BRKR", "DRH", "APA", "LAUR", "BIIB",
]
OUT = Path("/tmp/rank_20260730b.json")
QUAL_SEEDS = [
    Path("/tmp/qual_20260729_full.json"),
    Path("/tmp/qual_20260728_full.json"),
    Path("/tmp/qual_20260727_full.json"),
]
QUAL_OUT = Path("/tmp/qual_20260730b_full.json")
...
def load_qual_base():
    ...
    for p in QUAL_SEEDS:
        ...
        for k, v in data.items():
            if k not in base or len(str(v.get("item", ""))) > len(str(base[k].get("item", ""))):
                base[k] = v
    return base
...
def main():
    ...
    for i, r in enumerate(rows, 1):
        r["rank"] = i
    OUT.write_text(
        json.dumps({"as_of": ASOF.isoformat(), "rows": rows, "failed": failed, "formula": FORMULA}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
