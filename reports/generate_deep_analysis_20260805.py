#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""NASDAQ 심층분석 PDF — 2026-08-05 (60종목 · 매수시나리오 A′ 점검) + Chase 스냅샷 + 기술적분석 훅."""
...
RANK_PATH = Path("/workspace/reports/rank_20260805.json")
QUAL_PATH = Path("/workspace/reports/qual_20260805.json")

HTML_OUT = Path("/workspace/reports/NASDAQ_Deep_Analysis_2026-08-05.html")
PDF_WORKSPACE = Path("/workspace/reports/NASDAQ_Deep_Analysis_2026-08-05.pdf")
PDF_ARTIFACT = Path("/opt/cursor/artifacts/NASDAQ_Deep_Analysis_2026-08-05.pdf")
PDF_ASSETS = Path("/workspace/assets/NASDAQ_Deep_Analysis_2026-08-05.pdf")
CHASE_OUT = Path("/workspace/reports/chase_rr_20260805.json")
...
CHASE_DETAIL = {
    ...
    "ECPG": "최상·품질A·추가금지 — RR·Beat 상위·08-05 EARN_D5. 천장권·추가금지.",
    ...
    "TXG": "하·과열·품질C·보유3주·추가금지 — 비중OVER·평단↓·08-06 EARN_D5. 물타기·추격 금지.",
    ...
    "LASR": "중상·품질B·보유·추가금지 — 08-06 EARN_D5. 추격·추가 금지.",
    "RELY": "중·품질C·보유·추가금지 — 08-05 EARN_D5. 홀드·갭추격 금지.",
...
def load_buy_scenarios() -> list[dict]:
    p = Path("/workspace/reports/buy_scenarios_20260805.json")
    if not p.exists():
        return []
...
def main() -> None:
    ...
    <section class="cover">
      ...
      <div class="portfolio-box" style="text-align:left;max-width:560px;margin:16px auto;">
        <b>포트 실행 메모 (8/5 앱스냅)</b><br/>
        보유: ECPG2 · AMRX9 · LASR2 · TXG3 · NESR4 · CMPR1 · RELY3 · 주식 $947 · 현금 $566 · SCHD 청산<br/>
        등급한도: A≤18% · B≤10% · C≤6% · 금지=ADD0 · 절대천장20% · Top3≤50%<br/>
        전원 NO_ADD(유효금지) · 오늘 ECPG/RELY 실적 · 내일 LASR/TXG 실적 — 갭추격·물타기 금지<br/>
        비중OVER: ECPG~20% · LASR~16% · AMRX~18% · TXG~15% · NESR~12% · CMPR~11%<br/>
        유니버스 60종(CMPR 미포함·보유만) · A′/SOFT는 0장 표 · 실행은 포폴() 3레이어·등급한도 후
      </div>
    </section>
    {buy_scenario_section(scenarios)}
    <h2>Surprise / Chase 요약</h2>
    <p class="small">정렬: Chase RR. 가격·PT는 {DATE_TAG} 라이브. 60종 · 신규/교체: FROG·VISN·KALU·MRCY·AEIS·TER·XMTR·PLPC·ENTG·ANDE·ENLT·AEHR·NEOG·CRDO·BELFB·ADEA·NBIS 등.<br/>
    ...
    <section class="ticker-section">
      ...
      <p class="small">다음 단계: 기술적분석() ← chase_rr_20260805.json</p>
    </section>
    </body></html>
    """
