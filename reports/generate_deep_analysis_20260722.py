#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""NASDAQ 심층분석 PDF — 2026-07-22 (44종목) + Chase 스냅샷 + 기술적분석 훅."""
...
RANK_PATH = Path("/tmp/rank_20260722.json")
QUAL_PATH = Path("/tmp/qual_prev.json")

HTML_OUT = Path("/workspace/reports/NASDAQ_Deep_Analysis_2026-07-22.html")
PDF_WORKSPACE = Path("/workspace/reports/NASDAQ_Deep_Analysis_2026-07-22.pdf")
PDF_ARTIFACT = Path("/opt/cursor/artifacts/NASDAQ_Deep_Analysis_2026-07-22.pdf")
CHASE_OUT = Path("/workspace/reports/chase_rr_20260722.json")
...
# Portfolio-aware Chase label overrides (final display)
LABEL_OVERRIDE = {
    "SLS": "상·위성·바이너리",
    "VISN": "상·위성",
    "ECPG": "최상·코어보유",
...
def main() -> None:
    ...
    <section class="cover">
      ...
      <div class="portfolio-box" style="text-align:left;max-width:520px;margin:24px auto;">
        <b>포트 실행 메모 (7/22 입금일)</b><br/>
        코어 유지: ECPG·ASTH · INDV 추가금지<br/>
        이벤트: NEO ≫ AMRX · ALKS · COCO(D-1)<br/>
        NESR: $25–27 밴드만 · 현재 고점대 대기<br/>
        버퍼 $300 · BTSG/과열군 추격 금지
      </div>
    </section>
    ...
    for p in (PDF_WORKSPACE, PDF_ARTIFACT):
