...
ASOF = date(2026, 7, 31)
TICKERS = [
    "TXG","CDNA","TVTX","LIND","FTRE","VCYT","ADPT","KRYS","AMRX","JAZZ",
    "INDV","FA","GPRE","ROKU","SHOO","RELY","CRSR","INCY","STLD","KNSA",
    "SBLK","VTRS","CASY","PLPC","ECPG","BUSE","EXEL","NESR","CBRL","SENEA",
    "PEBO","FTNT","BTSG","DDOG","HST","PGNY","CHEF","CNXN","SCSC","PAGP",
    "LQDA","DNTH","CLMT","PTGX","ATLC","SEPN","RAPP","BRKR","ALKS","DRH",
    "TFSL","APA","ACHC",
]
OUT = Path("/tmp/rank_20260731.json")
QUAL_SEEDS = [
    Path("/workspace/reports/qual_20260730b.json"),
    Path("/tmp/qual_20260730b_full.json"),
    Path("/tmp/qual_20260730_full.json"),
    Path("/tmp/qual_20260729_full.json"),
]
QUAL_OUT = Path("/tmp/qual_20260731_full.json")
FORMULA = "epsBeat + upside - near_high - max(prem,0) + clip(avgSurp)"

SECTOR_KO = {
    "Technology": "IT·기술", "Healthcare": "헬스케어", "Financial Services": "금융",
    "Consumer Cyclical": "경기소비", "Consumer Defensive": "필수소비", "Energy": "에너지",
    "Industrials": "산업재", "Basic Materials": "소재", "Real Estate": "부동산",
...
def fetch_one(t):
    ...
    row = {
        "t": t, "name": info.get("shortName") or info.get("longName") or t,
        "sector": info.get("sector") or "", "industry": info.get("industry") or "",
        "px": px, "h": h52, "l": l52, "ptA": ptA,
        "ptL": float(ptL) if ptL else None, "ptH": float(ptH) if ptH else None,
        "rec": info.get("recommendationKey") or "", "mcap": info.get("marketCap"),
        "earnDate": _earn_date(info, tk),
        "epsBeat": beats, "epsN": n, "avgSurp": avg_surp,
        "upside": upside, "near_high": near_high, "prem": prem,
    }
    ups = upside or 0.0
...
def stub_qual(t, row, info):
    ...
    px = row.get("px") or pt
    return {
        "name": name, "sector": sec, "feat": feat,
        "item": f"{name}. 한줄: {summary}",
        "fin": f"EPS {beats}/{row.get('epsN') or 0} · upside {ups*100:+.0f}%" if ups is not None else f"EPS {beats}/{row.get('epsN') or 0}",
        ...
        "entry": entry, "drop": "가이던스 컷", "chase": chase,
...
def main():
    ...
    for t in TICKERS:
        try:
            r, info = fetch_one(t)
            if r is None:
                failed.append(t); print("FAIL", t); continue
            rows.append(r)
            if t not in qual or not qual[t].get("item"):
                qual[t] = stub_qual(t, r, info)
            ups = f"{r['upside']:+.1%}" if r["upside"] is not None else "—"
            print(f"OK {t:5} px={r['px']:.2f} ups={ups} beat={r['epsBeat']}/{r['epsN']} raw={r['chase_raw']:.3f}")
        except Exception as e:
            failed.append(t); print("ERR", t, e)
    rows.sort(key=lambda x: x["chase_raw"], reverse=True)
    for i, r in enumerate(rows, 1):
        r["rank"] = i
    OUT.write_text(json.dumps({"as_of": ASOF.isoformat(), "rows": rows, "failed": failed, "formula": FORMULA}, ensure_ascii=False, indent=2), encoding="utf-8")
