#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""NASDAQ 심층분석 PDF — 2026-07-30 PM (50종목 · 매수시나리오 점검) + Chase 스냅샷 + 기술적분석 훅."""
...
RANK_PATH = Path("/workspace/reports/rank_20260730b.json")
QUAL_PATH = Path("/workspace/reports/qual_20260730b.json")

HTML_OUT = Path("/workspace/reports/NASDAQ_Deep_Analysis_2026-07-30b.html")
PDF_WORKSPACE = Path("/workspace/reports/NASDAQ_Deep_Analysis_2026-07-30b.pdf")
PDF_ARTIFACT = Path("/opt/cursor/artifacts/NASDAQ_Deep_Analysis_2026-07-30b.pdf")
PDF_ASSETS = Path("/workspace/assets/NASDAQ_Deep_Analysis_2026-07-30b.pdf")
CHASE_OUT = Path("/workspace/reports/chase_rr_20260730b.json")
...
def load_buy_scenarios() -> list[dict]:
    p = Path("/workspace/reports/buy_scenarios_20260730b.json")
    if not p.exists():
        return []
...
def main() -> None:
    ...
    CHASE_OUT.write_text(
        json.dumps({"as_of": DATE_STR, "source": "심층분석-PM", "rows": chase_rows, "buy_scenarios": scenarios}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    ...
    <title>NASDAQ 심층분석 {DATE_TAG} PM</title><style>{build_css()}</style></head><body>
    <section class="cover">
      <h1>NASDAQ 심층분석</h1>
      <p class="sub">Chase Risk-Reward · 매수시나리오 A/B 점검 · 50종목 · 오후 갱신</p>
      <p class="meta">기준일 {DATE_TAG} PM · 라이브 가격/PT/실적일 · BII→BIIB 해석 · 종료 후 기술적분석 자동</p>
      ...
      <div class="portfolio-box" style="text-align:left;max-width:560px;margin:16px auto;">
        <b>포트 실행 메모 (7/30 PM)</b><br/>
        보유: ECPG · AMRX · LASR · SCHD · NESR · 현금 높게 유지<br/>
        LASR 무효~$60.50 이탈 → E형 청산/축소 우선<br/>
        오늘 실적창: AMRX·VCYT·LAUR·CDNA·DRH — 추가매수·갭베팅 금지<br/>
        08-04 INDV · 08-05 RELY/ECPG/HST/APA/SBLK · 08-06 ROKU/RAPP/TXG/PGNY — EARN_D5<br/>
        워치만: RELY·APA·SBLK·ALKS · 추격 금지
      </div>
    </section>
    {buy_scenario_section(scenarios)}
    <h2>Surprise / Chase 요약</h2>
    <p class="small">정렬: Chase RR. 가격·PT는 {DATE_TAG} PM 라이브. 티커 BII는 BIIB로 처리.</p>
    <table>
    ...
    <section class="ticker-section">
      ...
      <p class="small">다음 단계: 기술적분석() ← chase_rr_20260730b.json</p>
    </section>
    </body></html>
    """
