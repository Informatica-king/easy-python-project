...
<p class="small">생성: 수익구조분석() · 티커 SYRE · 기준 {ASOF} · WeasyPrint + NanumGothic · sepa.rev_compete · sepa.rev_price_chart</p>
</body></html>
"""
...
def main() -> None:
    ...
    bundle, cpaths = build_compete_charts("SYRE", CHART_DIR)
    ...
    compete_html = render_compete_section_html(
        ...
        img_b64_fn=img_b64,
        gloss_fn=gloss,
        easy_share="<div class='easy'><b>쉽게:</b> SYRE는 IBD 임상 피어셋에서 관심·시총이 빠르게 커진 축이다. 상업 IBD(ABBV 등) 절대 점유와는 다른 레이어다.</div>",
        easy_mix="<div class='easy'><b>쉽게:</b> 제품매출 $0. 보이는 건 레거시 마일스톤·이자다. DNTH는 이자 비중, RAPP는 협업 비중이 더 크다.</div>",
        gloss_share=[("임상 mindshare", "시총·파이프라인 관심의 상대 비중(추정)."), ("상업 IBD", "이미 파는 약 — 임상사 점유와 직접 비교 금지.")],
        gloss_mix=[("Collab/Milestone", "라이선스·레거시 마일스톤 손익."), ("Interest", "현금 운용 이자.")],
    )
    ...
    print(f"compete={bundle.ticker if bundle else None}\nSYRE rev px={PX} pt={PT} upside={UPSIDE*100:.1f}% near_hi={NEAR_HI*100:.0f}%")
