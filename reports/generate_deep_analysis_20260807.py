#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""NASDAQ 심층분석 PDF — 2026-08-07 (53종목 · 매수시나리오 A′ 점검) + Chase 스냅샷 + 기술적분석 훅."""
...
RANK_PATH = Path("/workspace/reports/rank_20260807.json")
QUAL_PATH = Path("/workspace/reports/qual_20260807.json")

HTML_OUT = Path("/workspace/reports/NASDAQ_Deep_Analysis_2026-08-07.html")
PDF_WORKSPACE = Path("/workspace/reports/NASDAQ_Deep_Analysis_2026-08-07.pdf")
PDF_ARTIFACT = Path("/opt/cursor/artifacts/NASDAQ_Deep_Analysis_2026-08-07.pdf")
PDF_ASSETS = Path("/workspace/assets/NASDAQ_Deep_Analysis_2026-08-07.pdf")
CHASE_OUT = Path("/workspace/reports/chase_rr_20260807.json")
...
LABEL_OVERRIDE = {
    "ALKS": "중상·SOFT·워치",
    "ECPG": "최상·품질A·보유·추가금지·실적직후",
    "ROKU": "상·08-06",
    "HST": "중상·08-05·고점",
    "INDV": "중·청산후워치·08-04",
    "TXG": "하·과열·품질C·보유3주·추가금지·08-06",
    "APA": "중상·08-05",
    "ADPT": "중·SOFT·실적직후",
    "SBLK": "중상·08-05·고점주의",
    "CASY": "중·SOFT·고점주의",
...
CHASE_DETAIL = {
    ...
    "RELY": "중·품질C·보유·추가금지 — 8/5 실적직후 소화. 홀드·갭추격 금지.",
...
def load_buy_scenarios() -> list[dict]:
    p = Path("/workspace/reports/buy_scenarios_20260807.json")
    if not p.exists():
        return []
...
def main() -> None:
    ...
    <section class="cover">
      <h1>NASDAQ 심층분석</h1>
      <p class="sub">Chase Risk-Reward · 매수시나리오 A/B 점검 · 53종목 · A′ 게이트</p>
      ...
      <div class="portfolio-box" style="text-align:left;max-width:560px;margin:16px auto;">
        <b>포트 실행 메모 (8/7 · 스냅 8/6)</b><br/>
        보유: ECPG2 · AMRX9 · LASR2 · TXG3 · NESR4 · CMPR1 · RELY3 · 주식~$941 · 현금 $566<br/>
        등급한도: A≤18% · B≤10% · C≤6% · 금지=ADD0 · 절대천장20% · Top3≤50% · 전원 NO_ADD<br/>
        LASR/TXG/ROKU 8/6 AMC 소화(KST 8/7 새벽 이후) · 갭추격·물타기 금지<br/>
        유니버스 53종(LASR·CMPR 미포함·보유만) · A′ 보유 히트=NO_ADD · 실행은 포폴() 3레이어·등급한도<br/>
        PDF 다운로드: GitHub Release sepa-deep-YYYYMMDD (artifacts 로컬 경로 불가)
      </div>
    </section>
    {buy_scenario_section(scenarios)}
    <h2>Surprise / Chase 요약</h2>
    <p class="small">정렬: Chase RR. 가격·PT는 {DATE_TAG} 라이브. 53종 · 신규: LGND·MRCY·SBLK·SITM·SYRE·AVT·DGII·NSIT·VSAT·BLMN·ALNT·PLSE·LFST·VCTR·ZD·SRRK·MNST.<br/>
    ...
    <section class="ticker-section">
      ...
      <p class="small">다음 단계: 기술적분석() ← chase_rr_20260807.json</p>
    </section>
    </body></html>
    """
