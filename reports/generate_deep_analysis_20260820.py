#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""NASDAQ 심층분석 PDF — 2026-08-20 (53종목 · 매수시나리오 A′ 점검) + Chase 스냅샷 + 기술적분석 훅."""
...
RANK_PATH = Path("/workspace/reports/rank_20260820.json")
QUAL_PATH = Path("/workspace/reports/qual_20260820.json")

HTML_OUT = Path("/workspace/reports/NASDAQ_Deep_Analysis_2026-08-20.html")
PDF_WORKSPACE = Path("/workspace/reports/NASDAQ_Deep_Analysis_2026-08-20.pdf")
PDF_ARTIFACT = Path("/opt/cursor/artifacts/NASDAQ_Deep_Analysis_2026-08-20.pdf")
PDF_ASSETS = Path("/workspace/assets/NASDAQ_Deep_Analysis_2026-08-20.pdf")
CHASE_OUT = Path("/workspace/reports/chase_rr_20260820.json")
...
def load_buy_scenarios() -> list[dict]:
    p = Path("/workspace/reports/buy_scenarios_20260820.json")
    if not p.exists():
        return []
...
def main() -> None:
    ...
    <section class="cover">
      <h1>NASDAQ 심층분석</h1>
      <p class="sub">Chase 본선 · 타이밍 GO · 확정 EARN_D5 · 53종 · 군인 프리마켓 실행</p>
      ...
      <div class="portfolio-box" style="text-align:left;max-width:560px;margin:16px auto;">
        <b>포트 실행 메모 (8/14 스냅)</b><br/>
        보유: TXG4 · ECPG2 · CMPR2 · AMRX11 · FTNT1 · ADAP2 · SCSC2 · 주식~$1,200 · 현금합≈$467<br/>
        ADAP 신규 · AMRX+1 · SCSC−1 · 등급한도 A≤18% B≤10% C≤6% · 전원 NO_ADD · Top3≈52.5%<br/>
        SCSC 확정 EARN~08-20(D5 창) · CMPR/SCSC stop 경계 · 실행창 17:30~20:55 지정가<br/>
        유니버스 53종 · 본선∩GO 강조 · PDF=GitHub Release만
      </div>
    </section>
    {buy_scenario_section(scenarios, exec_hits=exec_hits)}
    <h2>Surprise / Chase 요약</h2>
    <p class="small">정렬: Chase RR. 가격·PT는 {DATE_TAG} 라이브. 53종 · 신규/복귀: AMGN·PRAX·ANAB·ALKS·DRH·VSAT·NVCR·FTRE·TXG 등.<br/>
    ...
    <section class="ticker-section">
      ...
      <p class="small">다음 단계: 기술적분석() ← chase_rr_20260820.json</p>
    </section>
    </body></html>
    """
    ...
    try:
        ...
        notes = (
            f"## NASDAQ 심층분석 ({DATE_TAG}) · {len(order)}종 · 53-ticker universe\n\n"
            ...
            f"- 본선∩GO: ALKS·DRH · ALKS=ta_watchlist NO_ADD · DRH=분할OK 후보\n"
        )
