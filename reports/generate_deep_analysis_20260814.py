#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""NASDAQ 심층분석 PDF — 2026-08-14 (52종목 · 매수시나리오 A′ 점검) + Chase 스냅샷 + 기술적분석 훅."""
...
RANK_PATH = Path("/workspace/reports/rank_20260814.json")
QUAL_PATH = Path("/workspace/reports/qual_20260814.json")

HTML_OUT = Path("/workspace/reports/NASDAQ_Deep_Analysis_2026-08-14.html")
PDF_WORKSPACE = Path("/workspace/reports/NASDAQ_Deep_Analysis_2026-08-14.pdf")
PDF_ARTIFACT = Path("/opt/cursor/artifacts/NASDAQ_Deep_Analysis_2026-08-14.pdf")
PDF_ASSETS = Path("/workspace/assets/NASDAQ_Deep_Analysis_2026-08-14.pdf")
CHASE_OUT = Path("/workspace/reports/chase_rr_20260814.json")
...
def load_buy_scenarios() -> list[dict]:
    p = Path("/workspace/reports/buy_scenarios_20260814.json")
    if not p.exists():
        return []
...
def main() -> None:
    ...
    <section class="cover">
      <h1>NASDAQ 심층분석</h1>
      <p class="sub">Chase 본선 · 타이밍 GO · 확정 EARN_D5 · 52종 · 군인 프리마켓 실행</p>
      ...
      <div class="portfolio-box" style="text-align:left;max-width:560px;margin:16px auto;">
        ...
        유니버스 52종 · 본선∩GO 강조 · PDF=GitHub Release만
      </div>
    </section>
    {buy_scenario_section(scenarios, exec_hits=exec_hits)}
    <h2>Surprise / Chase 요약</h2>
    <p class="small">정렬: Chase RR. 가격·PT는 {DATE_TAG} 라이브. 52종 · 신규/교체: FORM·COHU·ATRO·TER·IART·STX·SAIC·SITM·DDOG 등.<br/>
    ...
    <section class="ticker-section">
      ...
      <p class="small">다음 단계: 기술적분석() ← chase_rr_20260814.json</p>
    </section>
    </body></html>
    """
    ...
    try:
        ...
        notes = (
            f"## NASDAQ 심층분석 ({DATE_TAG}) · {len(order)}종 · 52-ticker universe\n\n"
