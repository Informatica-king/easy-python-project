...
> **문서 버전**: v0.1
> **상태**: VCP 파라미터 수치 확정 대기 중 (사용자 입력 필요 — §4.4 파라미터 표)
> **최종 수정일**: 2026-07-15
...
| `contraction_decay` | 수축 깊이 감소 비율 상한 | 0.75 | 0.5(엄격)~1.0(감소만 요구) | |
...
- 개인 연구·비상업 용도로는 문제없으나, 상업 서비스화 시에는 라이선스된 유료 데이터로 교체 필요
...
1. **§4.4 VCP 파라미터 확정값** — 표의 빈 칸 (미입력 시 권장값으로 시작)
2. Trend Template의 `trend_days`(1개월 vs 4~5개월), `rs_rank_min`(70/80/90) 선호값
3. 유니버스 추가 필터: 최소 주가(예: $10 이상 — 미너비니는 저가주 회피), 최소 일평균 거래대금
